...
Router.post('/getGoods',(req,res)=>{
  ...
  .then((data)=>{
  	...
  	obj.total=data.length
  	  return Goods.find().sort({_id:-1}).limit(Number(pagesize)).skip((Number(page)-1)*Number(pagesize))
  })
  .then((data)=>{
  	//处理的是第几页的几条数据
  	obj.goodslist=data
...
Router.post('/findGoodsByKw',(req,res)=>{
 ...
 let key=req.body.cha;
  let obj={}
  Goods.find({$or:[{name:{$regex:key}},{desc:{$regex:key}}]})
  .then((data)=>{
    // 获取总条数
    obj.total=data.length
      return Goods.find({$or:[{name:{$regex:key}},{desc:{$regex:key}}]}).sort({_id:-1}).limit(Number(pagesize)).skip((Number(page)-1)*Number(pagesize))
  })
  .then((data)=>{
    obj.goodslist=data
  	res.send({err:0,msg:'查询成功',data:obj})
